...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.cnn_dim = int(dim * cnn_channel_ratio)

        self.norm1 = norm_layer(dim)
        
        if dim - self.attn_dim > 0:
            self.filter = AFNO2D(
                dim - self.attn_dim - self.cnn_dim,
                num_blocks,
                sparsity_threshold,
        ...
        if self.cnn_dim > 0:  
            self.cnn = nn.Conv2d(in_channels=self.cnn_dim, out_channels=self.cnn_dim, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        if self.attn_dim == 0:
            ...
            x = torch.concat([x_attn, x_afno, x_cnn], axis=-1)
...
class AFNOAttnParallelNet(nn.Module):
    ...
    def __init__(
        ...
        if not isinstance(attn_channel_ratio, list):
            ...
            self.cnn_channel_ratio = [cnn_channel_ratio] * depth
        else:
            self.attn_channel_ratio = attn_channel_ratio
            self.cnn_channel_ratio = cnn_channel_ratio
        ...
        self.gradpatch_embed = PatchEmbed(
            img_size=img_size,
            patch_size=self.patch_size,
            in_chans=49,
            embed_dim=128,
        ...
        self.grad_embed = nn.Parameter(torch.zeros(1, num_patches, 128))
        
        
        self.time_embed = TimeFeatureEmbedding(d_model=embed_dim, embed_type='fixed', freq='h')
        self.time_embed_grad = TimeFeatureEmbedding(d_model=128, embed_type='fixed', freq='h')
        ...
        self.blocks0 = nn.ModuleList(
            [
                Block(
                    ...
                    cnn_channel_ratio=self.cnn_channel_ratio[i],
                    mlp_ratio=mlp_ratio,
                    drop=drop_rate,
                    drop_path=dpr[i],
        ...
        self.blocks_grad0 = nn.ModuleList(
            [
                Block(
                    dim=128,
                    input_resolution=(self.h, self.w),
        ...
        self.blocks_grad1 = nn.ModuleList(
            [
                Block(
                    dim=128,
                    input_resolution=(self.h, self.w),
        ...
        self.headgrad = nn.Linear(
            128,
            48 * self.patch_size[0] * self.patch_size[1],
            bias=False,
        )
    ...
    def forward_tensor(self, x, grad, x_time):
        B = x.shape[0]
        x = self.patch_embed(x)
        
        grad = self.gradpatch_embed(grad)
             
        x = x + self.pos_embed + self.time_embed(x_time,x.shape[1])
        grad = grad + self.grad_embed + self.time_embed_grad(x_time,x.shape[1])
        
        x = self.pos_drop(x)

        x = x.reshape((B, self.h, self.w, self.embed_dim))
        grad = grad.reshape((B, self.h, self.w, 128))

        for block0 in self.blocks0:
            x = block0(x)
            
        for bg1 in self.blocks_grad0:
            grad = bg1(grad)
        ...
        grad = grad.reshape((b, h, w, p1, p2, 48))
        grad = grad.permute(0, 5, 1, 3, 2, 4)
        grad = grad.reshape((b, 48, h * p1, w * p2))
        grad = grad
    ...
    def forward(self, x, x_time, geo):
        ...
        for i in range(self.num_timestamps):    
            grad = self.spatial_gradient_builtin(out)
            input = torch.cat((out, geo), dim=1)
            in_grad = torch.cat((grad, geo), dim=1)            
            out, grad = self.forward_tensor(input, in_grad, x_time[:,i])
            y.append(out)
            g.append(grad)
