...
class TimeFeatureEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len):
        ...
        time_feature = time_feature.expand(-1,  seq_len, -1)
        return time_feature
...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.value_embedding = PatchEmbed(img_size=(440,408),
            patch_size=(8,8),
            in_channels=24,
            embed_dim=768,)
        self.pos_embedding = nn.Parameter(torch.zeros(4, 2805 , 768))
    ...
    def forward(self, x, x_mark):
        ...
        B,L,C,W,H = x.shape
        x = x.reshape(-1,C,W,H)
        x_mark = x_mark.reshape(B*L,-1)
        valueX = self.value_embedding(x)
        seq_len =  valueX.shape[1]

        # 多个输入输出的时候这里要改
        x = valueX +self.pos_embedding + self.temporal_embedding(x_mark, seq_len)
