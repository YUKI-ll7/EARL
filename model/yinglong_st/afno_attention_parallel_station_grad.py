...
class AFNOAttnParallelNet(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            ...
            embed_dim=embed_dim-grid_embed_dim,
        )
        ...
        self.station_embed = BatchGAT(img_size, patch_size, grid_feat =7, mesh_feat=7, grid_embed_dim=grid_embed_dim, dropout=0.0, alpha=0.1, nheads=num_heads)
        ...
        self.block_grad1 = nn.ModuleList(
            [
                Block(
                    dim=128,
                    input_resolution=(self.h, self.w),
        ...
        self.block_grad2 = nn.ModuleList(
            [
                Block(
                    dim=128,
                    input_resolution=(self.h, self.w),
    ...
    def forward_tensor(self, x_in, in_grad, x_time, station, adj):
        B = x_in.shape[0]
        x = x_in[:,:-2]
        x_station = self.station_embed(x_in[:,-7:], station, adj)
        x = self.patch_embed(x)
        
        grad = torch.cat([in_grad,x_in],dim=1)
        grad = self.gradpatch_embed(grad)
        
        x = torch.cat([x,x_station],dim=-1)
    ...
    def forward(self, xb, in_grad, geo, goes, station, x_time, adj):
    
        input = torch.cat([goes, xb, geo], dim=1)
        xt = self.forward_tensor(input, in_grad, x_time, station, adj)
        return xt
