...
class AFNOAttnParallelNet(nn.Module):
    ...
    def __init__(
        ...
        self.station_embed = BatchGAT(img_size, patch_size, grid_feat =7, mesh_feat=7, grid_embed_dim=grid_embed_dim, dropout=0.0, alpha=0.01, nheads=num_heads)
    ...
    def forward_tensor(self, x_in, x_time, station, adj):
        B = x_in.shape[0]
        x = x_in[:,:-2]
    ...
    def forward(self, xb, geo, goes, station, x_time, adj):
        input = torch.cat([goes, xb, geo], dim=1)
        xt = self.forward_tensor(input, x_time, station, adj)
        return xt
